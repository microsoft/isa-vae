...
def elbo_decomposition(vae, prior_dist, dataset_loader, use_cuda=True):
    n_samples = 10000
    ...
    for xs in dataset_loader:
        ...
        with torch.no_grad():
            if use_cuda:
                ...
                xs = Variable(xs)
        z_params_ = vae.encoder.forward(xs).view(batch_size, K, nparams)
        if vae.var_clipping:
            z_params = torch.cat((z_params_[..., 0].unsqueeze(-1), torch.clamp(z_params_[..., 1], vae.lowerbound_ln_var, vae.upperbound_ln_var).unsqueeze(-1)), dim=-1)
        else:
            z_params = z_params_
        ...
        for _ in range(S):
            ...
            if isinstance(vae.x_dist, dist.Normal):
                logpx += vae.x_dist.log_density_mu_sigma(xs, mu=x_params).view(batch_size, -1).data.sum()
            else:
    ...
    # nlogqz_condx, nlogpz = analytical_NLL(qz_params, vae.q_dist, prior_dist)
...
if __name__ == '__main__':
    ...
    def load_model_and_dataset(checkpt_filename):
        ...
        if args.dist == 'normal':
            ...
            q_dist = dist.Laplace()
        vae = VAE(z_dim=args.latent_dim, use_cuda=use_cuda, q_dist=q_dist, conv=args.conv)
        vae.load_state_dict(state_dict, strict=False)
        vae.eval()

        # dataset loader
        loader = setup_data_loaders(args, use_cuda=use_cuda)
        return vae, prior_dist, loader

    use_cuda = (args.gpu >= 0)
    if use_cuda:
        torch.cuda.set_device(args.gpu)
    vae, prior_dist, dataset_loader = load_model_and_dataset(args.checkpt, use_cuda=use_cuda)
    logpx, dependence, information, dimwise_kl, analytical_cond_kl, marginal_entropies, joint_entropy = \
        elbo_decomposition(vae, prior_dist, dataset_loader, use_cuda=use_cuda)
    torch.save({
        'logpx': logpx,
