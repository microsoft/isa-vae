...
from lib.adapters import LpNestedAdapter, StudentTAdapter

from elbo_decomposition import elbo_decomposition
from plot_latent_vs_true import plot_vs_gt_shapes, plot_vs_gt_faces, plot_vs_gt_cars3d
...
class ConvEncoderCars3d(nn.Module):
    def __init__(self, output_dim):
        super(ConvEncoderCars3d, self).__init__()
        self.output_dim = output_dim
...
class ConvDecoderCars3d(nn.Module):
    def __init__(self, input_dim):
        super(ConvDecoderCars3d, self).__init__()
        self.conv1 = nn.ConvTranspose2d(input_dim, 512, 1, 1, 0)  # 1 x 1
        self.bn1 = nn.BatchNorm2d(512)
        self.conv2 = nn.ConvTranspose2d(512, 64, 4, 1, 0)  # 4 x 4
        self.bn2 = nn.BatchNorm2d(64)
...
class ConvEncoderCelebA(nn.Module):
    def __init__(self, output_dim):
        super(ConvEncoderCelebA, self).__init__()
        self.output_dim = output_dim

        self.conv1 = nn.Conv2d(3, 64, 4, 2, 1) # 32 x 32
...
class ConvDecoderCelebA(nn.Module):
    def __init__(self, input_dim):
        super(ConvDecoderCelebA, self).__init__()
        self.conv1 = nn.ConvTranspose2d(input_dim, 512, 1, 1, 0) # 1 x 1
        self.bn1 = nn.BatchNorm2d(512)
        self.conv2 = nn.ConvTranspose2d(512, 256, 4, 1, 0) # 4 x 4
        self.bn2 = nn.BatchNorm2d(256)
        self.conv3 = nn.ConvTranspose2d(256, 128, 4, 2, 1) # 8 x 8
        self.bn3 = nn.BatchNorm2d(128)
        self.conv4 = nn.ConvTranspose2d(128, 128, 4, 2, 1) # 16 x 16
        self.bn4 = nn.BatchNorm2d(128)
        self.conv5 = nn.ConvTranspose2d(128, 64, 4, 2, 1) # 32 x 32
        self.bn5 = nn.BatchNorm2d(64)
        self.conv_final_mu = nn.ConvTranspose2d(64, 3, 4, 2, 1)
        self.act = nn.ReLU(inplace=True)
        
    def forward(self, z):
        batchsize = z.size(0)
        h = z.view(batchsize, -1, 1, 1)
        h = self.act(self.bn1(self.conv1(h)))
        h = self.act(self.bn2(self.conv2(h)))
        h = self.act(self.bn3(self.conv3(h)))
        h = self.act(self.bn4(self.conv4(h)))
        h = self.act(self.bn5(self.conv5(h)))
        img_mu = self.conv_final_mu(h).view(batchsize, 3, 64, 64).sigmoid()
        return img_mu


class VAE(nn.Module):
    def __init__(self, z_dim, use_cuda=False, q_dist=dist.Normal(), x_dist=dist.Bernoulli(),
                 include_mutinfo=True, tcvae=False, conv=False, mss=False, dataset='', mse_sigma=0.01, DIP = False, DIP_type = 2, lambda_od = 2.0, lambda_d = 2.0):
    ...
    def model_sample(self, prior_dist, batch_size=1):
        # sample from prior (value will be sampled by guide when computing the ELBO)
        prior_params = self._get_prior_params(batch_size)
        zs = prior_dist.sample(params=prior_params)
    ...
    def elbo(self, prior_dist, x, dataset_size, use_mse_loss=False, mse_sigma = 0.01):
        ...
        if isinstance(self.x_dist, dist.Normal):
            logpx = self.x_dist.log_density_mu_sigma(x, mu=x_params).view(batch_size, -1).sum(1)
        else:
            logpx = self.x_dist.log_density(x, params=x_params).view(batch_size, -1).sum(1)
            
        logpz = prior_dist.log_density(zs, params=prior_params).view(batch_size, -1).sum(1)
        logqz_condx = self.q_dist.log_density(zs, params=z_params).view(batch_size, -1).sum(1)

        elbo = logpx + logpz - logqz_condx

        if self.beta == 1 and self.include_mutinfo and self.lamb == 0 and not self.DIP:
...
def display_samples(model, prior_dist, x, vis, num_channels=1):
    global win_samples, win_test_reco, win_latent_walk

    # plot random samples
    sample_mu = model.model_sample(prior_dist, batch_size=100).sigmoid()
    sample_mu = sample_mu
    images = list(sample_mu.view(-1, num_channels, 64, 64).data.cpu())
    ...
    win_test_reco = vis.images(
        list(test_reco_imgs.contiguous().view(-1, num_channels, 64, 64).data.cpu()), 10, 2,
...
def parseISA(isalist, use_logp=False):
    print(isalist)
    p_ofs = 1.8
    pnested = []
    p0 = isalist[0]
    if use_logp:
        p0_param = torch.nn.Parameter(torch.tensor(np.log(p0 - p_ofs)).cuda())
        p0_param.requires_grad = True
        pnested.append(p0_param)
    else:
        p0_param = torch.nn.Parameter(torch.tensor(p0).cuda())
        p0_param.requires_grad = True
        pnested.append(p0_param)
    children = []
    for (p, n) in isalist[1]:
        if n > 0:
            if use_logp:
                p_param = torch.nn.Parameter(torch.tensor(np.log(p - p_ofs)).cuda())
                p_param.requires_grad = True
                nestedlayer = [p_param, [ [1.0] ] * n ]
                children.append(nestedlayer)
            else:
                p_param = torch.nn.Parameter(torch.tensor(p).cuda())
                p_param.requires_grad = True
                nestedlayer = [p_param, [ [1.0] ] * n ]
                children.append(nestedlayer)
    if len(children) == 0:
...
def backwardsParseISA(pnestedparam, use_logp=False):
    p_ofs = 1.8
    p1list = []
    p0_ = pnestedparam[0]
    if use_logp:
        p0 = p_ofs + np.exp(p0_.cpu().detach().numpy())
    else:
        p0 = p0_.cpu().detach().numpy()
    subspaces = pnestedparam[1]
    for subspace in subspaces:
        p1_ = subspace[0]
        if use_logp:
            p1 = p_ofs + np.exp(p1_.cpu().detach().numpy())
        else:
            p1 = p1_.cpu().detach().numpy()
        p1list.append(p1)
    return p0, p1list
...
def main():
    ...
    parser.add_argument('-dist', default='normal', type=str, choices=['normal', 'lpnorm', 'lpnested'])
    parser.add_argument('-n', '--num-epochs', default=50, type=int, help='number of training epochs')
    parser.add_argument('--num-iterations', default=0, type=int, help='number of iterations (overrides number of epochs if >0)')
    ...
    parser.add_argument('-p', '--pnorm', default=4.0/3.0, type=float, help='p value of the Lp-norm')
    parser.add_argument('--pnested', default='', type=str, help='nested list representation of the Lp-nested prior, e.g. [2.1, [ [2.2, [ [1.0], [1.0], [1.0], [1.0] ] ], [2.2, [ [1.0], [1.0], [1.0], [1.0] ] ], [2.2, [ [1.0], [1.0], [1.0], [1.0] ] ] ] ]')
    parser.add_argument('--isa', default='', type=str, help='shorthand notation of ISA Lp-nested norm, e.g. [2.1, [(2.2, 4), (2.2, 4), (2.2, 4)]]')
    ...
    parser.add_argument('--scale', default=1.0, type=float, help='scale of LpNested distribution')
    parser.add_argument('--q-dist', default='normal', type=str, choices=['normal', 'laplace'])
    parser.add_argument('--x-dist', default='bernoulli', type=str, choices=['normal', 'bernoulli'])
    ...
    parser.add_argument('--seed', default=-1, type=int, help='seed for pytorch and numpy random number generator to allow reproducibility (default/-1: use random seed)')
    parser.add_argument('--log_freq', default=200, type=int, help='num iterations per log')
    parser.add_argument('--use-mse-loss', action='store_true')
    parser.add_argument('--mse-sigma', default=0.01, type=float, help='sigma of mean squared error loss')
    parser.add_argument('--dip', action='store_true', help='use DIP-VAE')
    parser.add_argument('--dip-type', default=1, type=int, help='DIP type (1 or 2)')
    parser.add_argument('--lambda-od', default=2.0, type=float, help='DIP: lambda weight off-diagonal')
    parser.add_argument('--clip', default=0.0, type=float, help='Gradient clipping (0 disabled)')
    parser.add_argument('--test', action='store_true', help='run test')
    parser.add_argument('--trainingsetsize', default=0, type=int, help='Subsample the trainingset (0 use original training data)')
    ...
    # data loader
    train_loader = setup_data_loaders(args.dataset, args.batch_size, use_cuda=use_cuda, len_subset=args.trainingsetsize)
    ...
    vae = VAE(z_dim=args.latent_dim, use_cuda=use_cuda, prior_dist=prior_dist, q_dist=q_dist, x_dist=x_dist,
        include_mutinfo=not args.exclude_mutinfo, tcvae=args.tcvae, conv=args.conv, mss=args.mss, dataset=args.dataset, mse_sigma=args.mse_sigma,
        DIP = args.dip, DIP_type = args.dip_type, lambda_od = args.lambda_od, lambda_d = lambda_d)

    # setup the optimizer
    optimizer = optim.Adam([ {'params': vae.parameters()}, {'params': prior_dist.parameters(), 'lr': 5e-4}], lr=args.learning_rate)
    ...
    if not os.path.exists(trainingcurve_filename):
        with open(trainingcurve_filename,'w') as fd:
            fd.write('iteration,num_iterations,time,elbo_running_mean_val,elbo_running_mean_avg\n')
    ...
    while iteration < num_iterations and not nan_detected:
        for i, x in enumerate(train_loader):
            ...
            obj, elbo, logpx = vae.elbo(prior_dist, x, dataset_size, use_mse_loss=args.use_mse_loss, mse_sigma=args.mse_sigma)
            if utils.isnan(obj).any():
                print('NaN spotted in objective.')
                print('lpnested: {}'.format(prior_dist.prior.p))
                print("gradient abs max {}".format( max([g.abs().max() for g in gradients])))
            ...
            gradients = list(filter(lambda p: p.grad is not None, vae.parameters()))
            ...
            if iteration % args.log_freq == 0:
                ...
                print('[iteration %03d/%03d] time: %.2f \tbeta %.2f \tlambda %.2f \tobj %.4f \tlogpx %.4f training ELBO: %.4f (%.4f)' % (
                ...
                with open(os.path.join(args.save, 'trainingcurve.csv'),'a') as fd:
                    fd.write('{},{},{},{},{}\n'.format(iteration, num_iterations, time_, elbo_running_mean.val, elbo_running_mean.avg))

                if best_checkpoint_updated:
                    print('Update best checkpoint [iteration %03d] training ELBO: %.4f' % (best_checkpoint['iteration'], best_checkpoint['elbo']))
                ...
                if iteration % (10*args.log_freq) == 0:
                    utils.save_checkpoint({
                        ...
                        args.save, prefix='latest-optimizer-model-')
    ...
    dataset_loader = DataLoader(data_set, batch_size=1000, num_workers=0, shuffle=False)
    ...
    if not args.dataset=='celeba' and not args.dataset=='3dchairs':
        ...
        metric, metric_marginal_entropies, metric_cond_entropies = eval('disentanglement_metrics.mutual_info_metric_' + args.dataset)(vae, dataset_loader.dataset)
        torch.save({
            'args': args,
            'metric': metric,
        ...
        if args.dist == 'lpnested':
            if args.dataset == 'shapes':
                eval('plot_vs_gt_' + args.dataset)(vae, dataset_loader.dataset, os.path.join(args.save, 'gt_vs_grouped_latent.png'), eval_subspaces=True)
                ...
                }, os.path.join(args.save, 'disentanglement_metric_subspaces.pth'))
